...
# Create the specific folder for the Q-Learning bin experiments
output_folder = os.path.join("Assignment 1", "Question 2", "q_learning_bin_search_data")
...
class QLearningAgent:
    ...
    def update_q(self, state, action, reward, terminated, next_state):
        # Q-Learning is Off-Policy: it assumes the greedy action for the next state
...
def train_worker(params):
    ...
    for episode in range(n_episodes):
        ...
        # Q-Learning loop (no need to fetch next_action here)
        while not done:
...
if __name__ == "__main__":
    ...
    print(f"Starting Q-Learning runs: {len(seeds)} seeds x {len(bins_list)} bin configurations = {len(grid)} total tasks.")
    ...
    with ProcessPoolExecutor(max_workers=max_workers) as executor:
        ...
        for future in tqdm(as_completed(futures), total=len(grid), desc="Training Q-Learning Agents"):
